    ...
    checkLowest(price, symbol, row) {
        ...
        if (price < row.lowest) {
          ...
          this.checkedHighLow[symbol].lowest = price;
           ...
           this.send('Lowest price met',row.name + ' is currently at it lowest price at ' + this.CURRENCY + Util.formatMoney(price,2) + ' and the previous lowest price was ' + this.CURRENCY + Util.formatMoney(row.lowest,2));
        }
    },
    checkHighest(price, symbol, row) {
        ...
        if (price > row.highest){
          ...
          this.checkedHighLow[symbol].highest = price;
           ...
           this.send('Highest price met',row.name + ' is currently at it highest price at ' + this.CURRENCY + Util.formatMoney(price,2) + ' and the previous highest price was ' + this.CURRENCY + Util.formatMoney(row.highest,2));
        }
    },
    send(title,message) {
        var p = new push({
